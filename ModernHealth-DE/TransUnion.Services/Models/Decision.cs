using DecisionEngine.Services.Models.Request;
using DecisionEngine.TunaService.Response.Model;
using System;
using System.Collections.Generic;
using System.Text;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace DecisionEngine.Services.Models
{
   public class Decision
    {

    }
    public class DecisionEngineRequest
    {
        public bool HardPull { get; set; }
        public string Id { get; set; }
        public string FirstName { get; set; }

        public string MiddleName { get; set; }

        public string LastName { get; set; }

        public DateTime? DateOfBirth { get; set; }
        public Address Address { get; set; }
        public string SsnNumber { get; set; }
        public double income { get; set; }

    }
    public class LoanOffersRequest
    {
        public long Income { get; set; }
        public int CrediScore { get; set; }
        public double MonthlyDebt { get; set; }
        public string SsnNumber { get; set; }
        public string FirstName { get; set; }
        public string LastName { get; set; }
        public long settingId { get; set; }
        public List<BankRule> bankRules { get; set; }
    }
    public class BankRule
    {
        public string ruleName { get; set; }
        public int? ruleValue { get; set; }
    }
    public class LoanOffersResponse
    {
        public string Status { get; set; }
        public int RulesCount { get; set; }
        public List<string> PassRules { get; set; }
        public List<string> FailRules { get; set; }

        public List<string> BankPassRules { get; set; }
        public List<string> BankFailRules { get; set; }

        public List<ScoreApr> Aprs { get; set; }
        public List<LoanOffer> Offers { get; set; }
        public List<LoanTerm> Terms { get; set; }
        public long Requestedloanamount { get; set; }
        public string Message { get; set; }
       /* [JsonPropertyName("CreditReportResult")]
        public DecisionEngine.Services.Models.Response.CreditReportResult creditReportResult { get; set; }*/

    }
   
    public class LoanOffer
    {
       
       // public double Apr { get; set; }
        public long FinalRequestedLoanAmount { get; set; }
        public long FinancedAmount { get; set; }
        public double FullNumberAmount { get; set; }
        public double InterestFeeAmount { get; set; }
        public double InterestRate { get; set; }
        public long LoanAmount { get; set; }
        public string LoanGrade { get; set; }
        public string OfferType { get; set; }

        public string OfferValue { get; set; }
        public int MaxCreditScore { get; set; }
        public int MinCreditSCore { get; set; }
        public double MaxDTI { get; set; }
        public double MinDTI { get; set; }
        public long MaximumAmount { get; set; }
        public long MinimumAmount { get; set; }
        public double FundingRate { get; set; }
        public double SalesPrice { get; set; }

        public double TotalLoanAmount { get; set; }

    }
    public class LoanTerm
    {
        public string TermDuration { get; set; }

        public string TermDescription { get; set; }
        public string GradeDescription { get; set; }
        public double MonthlyPayment { get; set; }
        public double MaxMonthlyPayment { get; set; }

    }
  
    public class GradeApr
    {
        public int GradeId { get; set; }
        public int OfferId { get; set; }
        public int MinScore { get; set; }
        public int MaxScore { get; set; }

        public long MinIncome { get; set; }
        public long MaxIncome { get; set; }
        public double Apr { get; set; }
    }

    public class ScoreApr
    {
        public int GradeId { get; set; }
        public int SettingId { get; set; }
        public int MinScore { get; set; }
        public int MaxScore { get; set; }

        public double Apr { get; set; }
    }
    public class ServiceCode
    {
        public string code { get; set; }
        public string description { get; set; }
     
    }
}
